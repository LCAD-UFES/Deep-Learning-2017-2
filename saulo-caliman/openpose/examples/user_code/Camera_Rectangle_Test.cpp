...
class UserOutputClass
{
    ...
    struct coords {
        ...
        float maxBdpY = 0; 
        float minBdpY = 0;
        std::string ShowCords(){
            return "min_X: "+std::to_string(minBdpX)+" max_X: "+std::to_string(maxBdpX)+"\nmin_Y: "+std::to_string(minBdpY)+" max_Y: "+std::to_string(minBdpX);
        }
    } ;
    double fps;
    cv::VideoCapture cam;
    std::vector<coords> people;
            ...
            if(people.size() > 0){
                for(int i = 0; i < people.size(); i++){
                    cv::rectangle(datumsPtr->at(0).cvOutputData,
                                    cv::Point(people[i].minBdpX,people[i].minBdpY),
                                    cv::Point(people[i].maxBdpX,people[i].maxBdpY),
                                    cv::Scalar(0,0,255),
                                    2,
                                    8,
                                    0);
    ...
    void printKeypoints(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
        ...
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            ...
            op::log("People beign recognized: "+std::to_string(poseKeypoints.getSize(0)));
            people.resize(poseKeypoints.getSize(0));
            op::log("Person pose keypoints:");
            for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
            {
                op::log("Person Number:"+std::to_string(person));
                people[person].minBdpX = std::numeric_limits<float>::max();
                people[person].minBdpY = std::numeric_limits<float>::max();
                people[person].maxBdpX = std::numeric_limits<float>::min();
                people[person].maxBdpY = std::numeric_limits<float>::min();
                ...
                for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++)
                {
                    ...
                    {
                        //A cada interação me dá 3 valores, X o Y e o score
                        auto x = poseKeypoints[{person, bodyPart, xyscore}];
                        switch(cnt){
                            case 0:
                                if(x > people[person].maxBdpX)
                                    people[person].maxBdpX = x;
                                if(x < people[person].minBdpX && x > 0)
                                    people[person].minBdpX = x;
                                cnt = cnt + 1;
                            break;
                            case 1:
                                if(x > people[person].maxBdpY)
                                    people[person].maxBdpY = x;
                                if(x < people[person].minBdpY && x > 0)
                                    people[person].minBdpY = x;
                                cnt = cnt + 1;
                            break;
                            case 2:
                                cnt = 0;
                ...
                op::log(people[person].ShowCords());
            }
            
            auto frame = datumsPtr->at(0).cvOutputData;
            const auto width = frame.size[1];
            const auto height = frame.size[0];
            const auto area = width * height;
            
            op::log("Screen width:"+std::to_string(width)+" height:"+std::to_string(height)+" area:"+std::to_string(area));
